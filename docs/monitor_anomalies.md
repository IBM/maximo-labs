...
9.   Monitor includes a many types of cards. Take a moment to see what cards were created for you in the instance dashboard.
...
11.  Insert the [predicted travel_time line chart card json](https://www.ibm.com/support/knowledgecenter/SSQR84_monitor/iot/dashboard/line_graph_json_ref.html) below into the `"cards":[]`.
    ...
    ```
    {
        ...
        "size": "LARGE",
        "title": "Travel Time Deviation Prediction",
        "type": "TIMESERIES"
    },
    ```

12.  Notice how the JSON `content >  series > dataSourceId`  fields don't have an aggregation ` "aggregator": "min",` like other line metrics.  As a result the `travel_time` metric line chart data will display raw data.
13.  Notice how the `range` json node lets Monitor know to only display the last 24 hours of data.
14.  Insert the anomalous [travel_time_anomaly line chart card json](https://www.ibm.com/support/knowledgecenter/SSQR84_monitor/iot/dashboard/line_graph_json_ref.html) below into the `"cards":[]`.
    ...
    ```
    {
        ...
        "size": "LARGE",
        "title": "Travel Time Deviation",
        "type": "TIMESERIES"
    },
    ```    
15.  Make sure to change all values with size `XLARGE` to `LARGE`  and `XSMALLWIDE`to `SMALLWIDE`.  Save your changes to the instance dashboard json file.   ![Import Instance Dashboard Json](img/i112.png) &nbsp;
16.  You can see the finished [Industrial_Robot_Instance_Travel_Time.json](here)  or in the [online Json editor]
17.  Click on `import` button, choose the `file` you updated with the line card click `open` to import your updated json file. ![Import Instance Dashboard Json](img/i110.png) &nbsp;
18.  Your instance dashboard should now look similar to the one below and have a Travel Time Card.
...
*  A correlated dependent target variable doesn’t come within range of predicted value.
...
5.  Repeat the above steps in this exercise adding, configuring and naming the anomaly scoring models in a similar way for the other models:  
...
4.  View the instance dashboard for any one of the Robots.  Analyze the anomaly high scores for each model to see which ones happen when the anomaly for  time_travel_anomaly happens.
...
6. Configure the alert to trigger an alert when the value of a metric-name `travel_time_fft_score` reaches 25.  An operator would be required to investigate each anomaly alert when they occur.  It is important to not set the score too high so that anomalies don't go undetected.  Vice versa don't set the alert expression to low so that alerts aren't triggered when there is no anomaly.   Monitor uses Python for its expression language  Using the format `df['metric-name'] > numeric threshold value`
7.  Set Alert Expression `Expression` value to `df['travel_time_fft_score']>48`
...
10.  Name the alert `travel_time_spectral_alert`  ![Configure Alert Expression](img/i88.png) &nbsp;
11. Repeat the above steps in this exercise adding, configuring and naming alerts for each anomaly scoring models.

Set Alert Expression `Expression` value to `df['travel_time_spectral_score']>48`

 Set `FFTbasedGeneralizedAnomalyScore2` in the function catalog.  Configure and name it `travel_time_fft_score`
7.  Search for `travel_time_ga_alert` with a score of  `travel_time_ga_score`
8.  Search for `travel_time_saliency_alert` in the function catalog.  Configure and name it `travel_time_saliency_score`
9.  Search for `travel_time_kmeans_alert` in the function catalog.  Configure and name it `travel_time_spectral_score`


Add alerts for each anomaly model score to your instance dashboard
```
{
